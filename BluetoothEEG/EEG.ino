...
void loopEeg() {
  ...
  if (sample_timer - timer >= sample_period) {
    ...
    if (t < SAMPLES) {
      ...
      ble_eeg_raw = double((eeg_filtered)*adc_conversion_factor * 10);
      ...
      if (ble_out_counter == SAMPLE_FREQ / 64) {  //output to ble less often because it slows.
